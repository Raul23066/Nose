...
export async function before(m, {conn, isAdmin, isBotAdmin}) {
  ...
  if (isAdmin && chat.antiLink && m.text.includes(grupo)) return m.reply('*𝐄𝐥 𝐀𝐧𝐭𝐢𝐥𝐢𝐧𝐤 𝐞𝐬𝐭𝐚 𝐀𝐜𝐭𝐢𝐯𝐨!! 𝐆𝐨𝐨𝐝 𝐬𝐨𝐬 𝐀𝐝𝐦𝐢𝐧!!*');
  if (chat.antiLink && isGroupLink && !isAdmin) {
    if (isBotAdmin) {
      const linkThisGroup = `https://chat.whatsapp.com/${await this.groupInviteCode(m.chat)}`;
      if (m.text.includes(linkThisGroup)) return !0;
    }
    await this.sendMessage(m.chat, {text: `* 𝐀𝐍𝐓𝐈 𝐋𝐈𝐍𝐊 *\n* ${user} 𝐈𝐧𝐜𝐮𝐦𝐩𝐥𝐢𝐬𝐭𝐞 𝐥𝐚𝐬 𝐑𝐞𝐠𝐥𝐚𝐬 , 𝐂𝐡𝐚𝐨! 𝐒𝐚𝐩𝐨 𝐇𝐩𝐭𝐚!*`, mentions: [m.sender]}, {quoted: m});
